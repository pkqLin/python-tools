...
class XiaomiSport:
    ...
    def change_step(self):
        ...
        if response.status_code == 200:
            print('【%s】账号修改步数为【%s】成功' % (self.phone[3:], self.step))
            return '【%s】账号修改步数为【%s】成功' % (self.phone[3:], self.step)
        else:
            print('【%s】账号修改步数失败' % self.phone[3:])
...
if __name__ == "__main__":
    ...
    while fail_num > 0:
        ...
        try:
            # 执行一键修改步数
            if count == 1:
            	# sys.argv[1]是从java中servlet里面的手机号,sys.argv[2]是从java中servlet里面的密码,sys.argv[3]是从java中servlet里面的步数
                XiaomiSport('18315303669', '67896789jj', 10000).one_click_change_step()
                count += 1
                break
